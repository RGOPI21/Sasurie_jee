        ...
        if (status === "submitted") {
            try {
                ...
                if (user && user.email) {
                    ...
                    const emailResult = await sendApplicationConfirmationEmail(user.email, user.fullName || "Applicant", data);
...
router.post("/leads", async (req, res) => {
    try {
        const { firstName, lastName, email, phone, interestAreas, message, source, meta, } = req.body ?? {};
        if (!firstName || !email) {
            return res
                .status(400)
